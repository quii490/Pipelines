...
这里是 RNA-seq、ATAC-seq、CUT&RUN/CUT&Tag/ChIP-seq 的统一中文使用手册。命令和参数保留英文，重点回答：**从什么数据开始、应该运行哪个入口、输出在哪里、结果能不能解释、失败后怎样安全续跑。**
...
| DE/差异 region 表 | 补图、注释、pathway/TE 图 | heatmap、annotation、motif | annotation、motif、heatmap |
...
| 蛋白/组蛋白结合、CUT&RUN、CUT&Tag、ChIP-seq | CUT&RUN pipeline（Draft，仍在开发） | [Quick Start](pipelines/cutrun/quick-start.md) |
